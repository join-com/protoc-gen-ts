...
export namespace Common {
  ...
  export class ExtraPkgMessage extends protobufjs.Message<ExtraPkgMessage> implements ConvertibleTo<IExtraPkgMessage> {
    ...
    public asInterface(): IExtraPkgMessage {
      const message = {
        ...this,
        birthDate: this.birthDate != null ? new Date((this.birthDate.seconds ?? 0) * 1000 + (this.birthDate.nanos ?? 0) / 1000000) : undefined,
      }
    ...
    public static fromInterface(this: void, value: IExtraPkgMessage): ExtraPkgMessage {
      const patchedValue = {
        ...value,
        birthDate: (value.birthDate != null) ? GoogleProtobuf.Timestamp.fromInterface({ seconds: Math.floor(value.birthDate.getTime() / 1000), nanos: value.birthDate.getMilliseconds() * 1000000 }) : undefined,
      }
